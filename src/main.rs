...
macro_rules! ensure_tonicity {
    ($check_fn:expr) => {{
        |args: &[RispExp]| -> Result<RispExp, RispErr> {
            ...
            let first = floats
                .first()
                .ok_or(RispErr::Reason("expected at least one float".to_string()))?;
            let rest = &floats[1..];
...
fn env_for_lambda<'a>(
    ...
    if ks.len() != arg_forms.len() {
        Err(RispErr::Reason(
            format!("expected {} arguments, got {} instead", ks.len(), arg_forms.len())
        ))
    }
    else {
                    ...
                    match first_eval {
                        ...
                        RispExp::Lambda(lambda) => {
                            let new_env = &mut env_for_lambda(
                                lambda.params_exp, arg_forms, env
                            )?;
